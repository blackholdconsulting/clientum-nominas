...
export async function POST(req: NextRequest) {
  try {
    ...
    // 4) (Opcional) Datos de organización (si existe tabla orgs)
    let org: any = null;
    ...
    // 5) Cálculos (bases, devengos/deducciones)
    const num = (x: any, d = 0) => {
    ...
    const pctSSTrab = num(p.ss_emp_pct ?? e.ss_emp_pct, 0);
    const pctSSEmp = num(e.ss_er_pct, 29.9); // % empresa (de tus capturas)
    ...
    // 8) Subir a Storage
    const bucket = "nominas"; // asegúrate de tener este bucket (privado) con políticas para usuario autenticado
    ...
    // 9) URL (si el bucket es público) o devuelve path interno
