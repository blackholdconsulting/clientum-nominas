...
export default async function ContractsPage({
  ...
  // Empleados para el selector de creación
  ...
  let listQuery = supabase
    .from("contracts")
    .select(
      "id, contract_type, start_date, end_date, status, employee:employees(id, full_name)"
    )
    .order("created_at", { ascending: false });

  if (q) listQuery = listQuery.ilike("employee.full_name", `%${q}%`);
  ...
  return (
    <main className="max-w-7xl mx-auto p-4 md:p-6">
      {/* Header con volver + CTA modelos */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <BackButton />
          <h1 className="text-2xl md:text-3xl font-semibold text-slate-900">
            Clientum Nóminas
            <span className="ml-2 text-sm font-normal text-slate-500 align-middle">
              Contratos
            </span>
          </h1>
        </div>
        <Link
          href="/plantillas"
          className="inline-flex items-center gap-2 rounded-md border bg-white px-3 py-2 text-sm text-slate-800 hover:bg-slate-50"
        >
      ...
      <Card className="shadow-clientum mb-6">
        <CardContent className="p-6">
          ...
          <p className="text-sm text-slate-500 mb-4">
            Genera un contrato básico y complétalo después con el modelo oficial.
          </p>
          ...
          >
            ...
            <div className="flex md:justify-end">
              <Button
                type="submit"
                className="bg-clientum-blue hover:bg-clientum-blueDark text-white inline-flex items-center gap-2"
              >
        ...
        <Card className="shadow-clientum">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 text-slate-700">
              ...
              <span className="text-sm">Total Contratos</span>
            </div>
