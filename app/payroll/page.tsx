...
// Paleta corporativa Clientum (puedes ajustar si tienes tokens propios)
const brand = {
  bg: "from-[#0E2A47] via-[#0F3D7A] to-[#1259C3]", // header gradiente
...
export default async function PayrollPage({
  ...
  // Lee todas las nóminas de ese año para el usuario actual (RLS lo aísla)
  ...
  if (error) {
    // En producción, mostramos estado amable
    return (
  ...
  // server action local: crear y redirigir
  async function createThenGo(formData: FormData) {
    "use server";
    const y = Number(formData.get("y"));
    const m = Number(formData.get("m"));
    await generatePayroll(y, m);
    redirect(`/payroll/period/${y}/${m}`);
  }

  // cambia el año (en SSR manteniendo ruta limpia)
  ...
  return (
    <div className="min-h-screen">
            ...
            <form action={changeYear} className="flex items-center gap-3">
              ...
              >
                {Array.from({ length: 6 }).map((_, i) => {
                  const y = now.getFullYear() - 2 + i; // dos atrás y tres adelante
                  return (
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        {/* Estado vacío anual (si no hay ninguna del año) */}
        {(payrolls?.length ?? 0) === 0 && (
          <div className="mt-12 rounded-xl border border-dashed border-slate-300 p-8 text-center">
            ...
            <p className="mt-2 text-slate-500">
              Crea la primera nómina del año y personalízala por empleado. Los
              importes se precargarán y podrás editar antes de finalizar.
            </p>
