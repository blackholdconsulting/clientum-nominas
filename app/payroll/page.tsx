...
import { createClient } from "@/lib/supabase/server"; // Ajusta si usas otra ruta
...
function formatNextPay(dateISO?: string | null) {
  if (!dateISO) return "-";
...
export default async function PayrollPage() {
  ...
  // RLS ya filtra por user_id; igualmente no pasamos user_id a consultas
  const [{ data: employees, error: empError }, { data: payrolls, error: payError }] =
    await Promise.all([
      supabase
        .from("employees")
        .select("id") // solo necesitamos el conteo
  ...
  if (empError) {
    // Muestra un fallback amable; no paramos la page
    console.error("Error employees:", empError.message);
  }
  if (payError) {
    console.error("Error payrolls:", payError.message);
  }

  const employeesCount = employees?.length ?? 0;
  const list: PayrollRow[] = payrolls ?? [];

  // Totales agregados (tolerantes a nulos/ausentes)
  ...
  // Próximo pago (si tienes next_pay_date, cogemos el primero válido)
  const nextPay =
        ...
        <div className="flex-1">
          <div className="relative">
            <input
              ...
              // (cliente) La lógica del filtro la harás en /payroll con interactividad si quieres
              readOnly
            />
            <svg
      ...
      <section className="rounded-xl border bg-white p-10 text-center text-sm text-muted-foreground">
        {list.length === 0 ? (
          ...
          <div className="text-left">
            {/* Si quieres mostrar la tabla, impléméntala aquí */}
            {/* Este bloque se deja preparado para que añadas tu tabla real */}
            <p className="text-foreground">
              (Aquí iría tu tabla de nóminas por periodo/estado)
            </p>
          </div>
        )}
      </section>
