...
export async function POST() {
  ...
  // último payroll_run en borrador
  const { data: run } = await supabase.from('payroll.payroll_runs')
    .select('id').eq('organization_id', orgId).eq('status','draft')
  ...
  const slips = (contracts ?? []).map(c => {
    ...
    return {
      organization_id: orgId, payroll_run_id: run.id, employee_id: c.employee_id,
      gross_salary: gross, ss_employee: ssEmp, ss_employer: ssEr, irpf, net_pay: net
    }
  })

  const { error: insErr, count } = await supabase
    .from('payroll.payslips').insert(slips, { count: 'exact' })
